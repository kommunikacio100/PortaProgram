...
router.post('/', (req, res)=>{
    ...
    ` created_by) `+
    ` VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`;
    let vehicle_plate_number1 = req.body.vehicle_plate_number1;
    let vehicle_country_code1 = req.body.vehicle_country_code1;
    let vehicle_plate_number2 = req.body.vehicle_plate_number2;
    let vehicle_country_code2 = req.body.vehicle_country_code2;
    let vehicle_plate_number3 = req.body.vehicle_plate_number3;
    let vehicle_country_code3 = req.body.vehicle_country_code3;
    let vehicle_empty_weight = req.body.vehicle_empty_weight;
    let vehicle_empty_time = req.body.vehicle_empty_time;
    let vehicle_created_by = req.body.vehicle_created_by;
    try{
...
router.put('/', (req, res)=>{
    var sql = `replace into vehicles ( id, item_number, name, units, `+
    ...
    let vehicle_empty_weight = req.body.vehicle_empty_weight;
    let vehicle_empty_time = req.body.vehicle_empty_time;
    let vehicle_modified = new Date().toISOString();
    let vehicle_modified_by = req.body.vehicle_modified_by;
    try{
        con.query(sql, [vehicle_id,
            vehicle_plate_number1, vehicle_country_code1, 
            vehicle_plate_number2, vehicle_country_code2, 
            vehicle_plate_number3, vehicle_country_code3, 
            vehicle_empty_weight, vehicle_empty_time,
            vehicle_modified, vehicle_modified_by], 
            function (err, result) {
            if (err){
                res.status(500)
                ...
                if (result.insertId>0){
                    console.log("put vehicle successfull. vehicle_id: " + result.insertId );
                    res.status(200);
                    result.status= "OK";
                    res.json( result);
                }else{
